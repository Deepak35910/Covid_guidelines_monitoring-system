...
image_BGR = cv2.imread('images/without-mask-UNI-3.jpg')
...
if len(results) > 0:
    ...
    for i in results.flatten():
        ...
        #it give negative coordinate making it 0
        if (y_min <0):
            y_min=0
        if (x_min <0):
            x_min=0
        img_pass=image_BGR[y_min:y_min+box_height,x_min:x_min+box_width]
        #print(x_min,box_width,y_min,box_height)
        #window_name = 'image'
        ...
        if label[0:7] == "No Mask":
            print_startX=ret_startX
            print_endX  =ret_endX  
            print_startY=ret_startY
            print_endY  =ret_endY  
            #print(print_startX,print_endX,print_startY,print_endY)
            
            #color_label=color_cloth(img_pass,ret_startX,ret_endX,ret_endY,ret_startY+ret_endY+ret_endY)
            color_label=color_cloth(img_pass,print_startX,print_endX,print_startY,ret_startY+ret_endY+ret_endY+ret_endY)
            color1=()
            ...
            cv2.rectangle(image_BGR, (x_min, y_min),
                          (x_min+box_width,y_min+box_height),color1, 2)
            ...
            #cv2.putText(image_BGR, label, (x_min, y_min - 5),
            #            cv2.FONT_HERSHEY_COMPLEX, 0.7, color1, 2)
            str1='people wearing a '+color_label+' color cloth wear a mask'
            #print(str1)
            #cv2.putText(image_BGR, str1, (x_min+10, y_min +10),
            #            cv2.FONT_HERSHEY_COMPLEX, 0.7, (255,255,0), 1)
            cv2.putText(image_BGR, str1, (10, 10),
